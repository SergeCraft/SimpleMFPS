...
public class HeadBob
{
    ...
    [Range(0, 5)] public float BobFrequency = 1.5f; // the base speed of the head bobbing (in cycles per metre)

    [Range(0, 1)] public float BobHeight = 0.3f; // the height range of the head bob
    ...
    [Range(0, 1)]
    public float
        heightSpeedMultiplier =
            0.35f; // the amount the bob height increases as the character's speed increases (for a good 'run' effect compared with walking)

    [Range(0, 1)]
    public float
        strideSpeedLengthen =
            0.35f; // the amount the stride lengthens based on speed (so that running isn't like a silly speedwalk!)

    // these control the amount of movement applied to the head when the character jumps or lands
    [Range(0, 5)] public float jumpLandMove = 1;

    [Range(0, 20)] public float jumpLandTilt = 10;
}
