...
public class FP_CameraLook : MonoBehaviour
{
    ...
    [Range(-35, -90)] public float minimumY = -60.0F; //Minimum look angle;

    [Range(35, 90)] public float maximumY = 60.0F; //Maximum look angle;
    ...
    private float headBobCycle; // the current position through the headbob cycle

    private float
        headBobFade; // the current amount to which the head bob position is being applied or not (it is faded out when the character is not moving)
    ...
    private readonly float springDampen = 0.8f;
    private readonly float springElastic = 1.1f;

    // Fields for simple spring calculation:
    private float springPos;
    private readonly float springPositionThreshold = 0.05f;
    private float springVelocity;
    private readonly float springVelocityThreshold = 0.05f;
    ...
    private void Update()
    {
        ...
        PlayerHead.localPosition = Vector3.Lerp(PlayerHead.localPosition, new Vector3(
            ...
            PlayerHead.localPosition.z), 0.15F * 3);
    ...
    private void FixedUpdate()
    {
        ...
        bobSwayFactor =
            Mathf.Sin(headBobCycle * Mathf.PI * 2 +
                      Mathf.PI * .5f); // sway is offset along the sin curve by a quarter-turn in radians
